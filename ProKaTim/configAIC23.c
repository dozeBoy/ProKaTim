...
\
0x0F9,  /* Set-Up Reg 2-.-       Left channel headphone volume control */   \
/* LRS     0          simultaneous left/right volume: disabled */ \
/* LZC     1          left channel zero-cross detect: enabled */ \
/* LHV     1111001    left headphone volume: 0 dB */             \
\
0x0f9, /* Set-Up Reg 3       Right channel headphone volume control */  \
...
\
0x0010, /* Set-Up Reg 4-.- default: 0x0011; bypass: 0x001C,      Analog audio path control */               \
/* X       0          reserved */                                \
...
\
0x0007, /* Set-Up Reg 5       Digital audio path control */              \
/* XXXXX   00000      reserved */                                \
/* DACM    0          DAC soft mute: disabled */                 \
/* DEEMP   11         deemphasis control: 48kHz */            \
/* ADCHP   1          ADC high-pass filter: enable */          \
\
...
void Config_DSK6713_AIC23(void)
{
	...
	MCBSP_start(hMcbsp0, MCBSP_XMIT_START | MCBSP_SRGR_START | MCBSP_SRGR_FRAMESYNC, 220); //-.- was macht _SRGR_Start and _Framesync
	//MCBSP_start(hMcbsp0, MCBSP_XMIT_START, 220); //-.-
...
//void set_aic23_register(MCBSP_Handle hMcbsp,unsigned short regnum, unsigned short regval)
{
	...
	/* zur Sicherheit maskieren auf 7 Bit */
	regnum &= 0x07f;


	/* warten */
	while (!MCBSP_xrdy(hMcbsp)); // -.- Negation eingefügt

	/* schreiben */
	MCBSP_write(hMcbsp, (regnum << 9) | regval); // -.- need 16bit
}
